...
} from "../apps/bitcraft-local/src/server/contributionAttributionRepair.mjs";
...
try {
  ...
  try {
    ...
    const result = options.mode === "dry-run"
      ? createContributionAttributionManifest(db, options.claimId)
      : applyContributionAttributionRepair(db, JSON.parse(readFileSync(manifestPath, "utf8")));
    if (options.mode === "dry-run") writeFileSync(manifestPath, `${JSON.stringify(result, null, 2)}\n`, { flag: "wx" });
