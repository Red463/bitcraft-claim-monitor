...
test("layered terrain store exposes whole-world overview coverage and detail fallback", async () => {
  ...
  let overviewReads = 0;
  const detail = {
    readManifest: async () => ({ generation: "2", generatedAt: "2026-08-11T12:00:00.000Z", regionIds: ["19"], bounds: { minX: 23040, minZ: 23040, maxX: 30720, maxZ: 30720 }, tileCount: 10, totalBytes: 100 }),
    readTile: async ({ z }) => {
      detailReads += 1;
  ...
  const overview = {
    readManifest: async () => ({ generation: "100", generatedAt: "2026-08-11T11:00:00.000Z", regionIds: ["3", "19"], bounds: { minX: 0, minZ: 0, maxX: 38400, maxZ: 38400 }, tileCount: 20, totalBytes: 200 }),
    readTile: async ({ z }) => {
      overviewReads += 1;
