    ...
    <section className="global-market-workspace market-overview">
      ...
      <section className="market-overview-section">
        ...
        {favoriteRows.length ? <div className="market-favorite-strip">{favoriteRows.map((row) => {
          ...
          return <button key={`${row.itemType}:${row.itemId}`} onClick={() => onOpenItem(itemShape(row))}><ItemLabel item={itemShape(row)} /><span>Sell <b>{row.bestSell == null ? "—" : `${formatNumber(row.bestSell)}g`}</b></span><span>Buy <b>{row.bestBuy == null ? "—" : `${formatNumber(row.bestBuy)}g`}</b></span><span>Spread <b>{spread == null ? "—" : `${formatNumber(spread)}g`}</b></span><small>{formatNumber(row.volume24h)} volume</small></button>;
      ...
      <div className="market-overview-grid">
        ...
        <section className="market-overview-section">
          ...
          <div className="market-ranking-list">{traded.slice(0, 10).map((row) => <button key={`${row.itemType}:${row.itemId}`} onClick={() => onOpenItem(itemShape(row))}><ItemLabel item={itemShape(row)} /><span>{formatCompactNumber(row.totalQuantity ?? row.volume24h)} units · {formatCompactNumber(row.totalValue)}g</span></button>)}</div>
        ...
        <section className="market-overview-section">
          ...
          <div className="market-ranking-list">{hubs.slice(0, 10).map((hub) => <button key={String(hub.claimId)} onClick={() => toNumber(hub.locationX) || toNumber(hub.locationZ) ? onShowMap({ name: hub.claimName, locationX: toNumber(hub.locationX), locationZ: toNumber(hub.locationZ) }, String(hub.regionId ?? "")) : undefined}><span><b>{hub.claimName ?? "Unknown settlement"}</b><small>{hub.regionName ?? `R${hub.regionId ?? "?"}`}</small></span><span>{formatCompactNumber(hub.tradedValue)}g · {formatNumber(hub.traders)} traders</span>{toNumber(hub.locationX) || toNumber(hub.locationZ) ? <MapPin size={14} /> : null}</button>)}</div>
        ...
        <section className="market-overview-section">
          ...
          <div className="market-activity-list">{activityRows.map((row, index) => <button key={String(row.id ?? `${row.itemType}:${row.itemId}:${row.createdAt}:${index}`)} onClick={() => onOpenItem(itemShape(row))}><ItemLabel item={itemShape(row)} /><span>{formatNumber(row.quantity)} @ {formatNumber(row.unitPrice ?? row.price)}g</span><small>{row.regionName ?? row.claimName ?? "Global market"} · {timeAgo(row.createdAt ?? row.timestamp)}</small><ArrowRight size={13} /></button>)}</div>
