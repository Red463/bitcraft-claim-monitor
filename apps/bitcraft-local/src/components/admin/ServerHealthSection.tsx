...
export function ServerHealthSection() {
  ...
  return <div className="admin-section server-health-page">
    ...
    <div className="server-health-runtime-grid"><section className="form-card"><h3>Craft Planner</h3><dl><div><dt>Latest calculation</dt><dd>{formatNumber(application.planner?.lastDurationMs)} ms</dd></div><div><dt>Latest payload</dt><dd>{bytes(application.planner?.lastResponseBytes)}</dd></div><div><dt>Cache hits</dt><dd>{formatNumber(application.planner?.cacheHits)}</dd></div><div><dt>In-flight reuse</dt><dd>{formatNumber(application.planner?.inflightReuse)}</dd></div></dl></section><section className="form-card"><h3>BitJita evidence</h3><dl><div><dt>Requests</dt><dd>{formatNumber(application.bitjita?.requests)}</dd></div><div><dt>Failures</dt><dd>{formatNumber(application.bitjita?.failures)}</dd></div><div><dt>Timeouts</dt><dd>{formatNumber(application.bitjita?.timeouts)}</dd></div><div><dt>Rate limits</dt><dd>{formatNumber(application.bitjita?.rateLimits)}</dd></div></dl></section><section className="form-card"><h3>Active incidents</h3><div className="server-health-incidents">{(data?.incidents ?? []).filter((item: AnyRecord) => item.state !== "recovered").slice(0, 6).map((item: AnyRecord) => <div key={item.incident_key}><span className="health-dot critical" /><strong>{String(item.incident_key).replaceAll("_", " ")}</strong><small>{item.state} · {item.consecutive_bad} samples</small></div>)}{!(data?.incidents ?? []).some((item: AnyRecord) => item.state !== "recovered") ? <p className="legend">No active incidents.</p> : null}</div></section></div>
