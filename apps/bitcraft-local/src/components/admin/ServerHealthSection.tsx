...
export function ServerHealthSection() {
  ...
  const routePerformance = application.routePerformance ?? {};
  const operationalHistory = data?.database?.operationalHistory ?? {};
  ...
  return <div className="admin-section server-health-page">
    ...
    <div className="server-health-runtime-grid"><section className="form-card"><h3>Craft Planner</h3><dl><div><dt>Latest calculation</dt><dd>{formatNumber(application.planner?.lastDurationMs)} ms</dd></div><div><dt>Latest payload</dt><dd>{bytes(application.planner?.lastResponseBytes)}</dd></div><div><dt>Cache hits</dt><dd>{formatNumber(application.planner?.cacheHits)}</dd></div><div><dt>In-flight reuse</dt><dd>{formatNumber(application.planner?.inflightReuse)}</dd></div></dl></section><section className="form-card"><h3>Active incidents</h3><div className="server-health-incidents">{(data?.incidents ?? []).filter((item: AnyRecord) => item.state !== "recovered").slice(0, 6).map((item: AnyRecord) => <div key={item.incident_key}><span className="health-dot critical" /><strong>{String(item.incident_key).replaceAll("_", " ")}</strong><small>{item.state} · {item.consecutive_bad} samples</small></div>)}{!(data?.incidents ?? []).some((item: AnyRecord) => item.state !== "recovered") ? <p className="legend">No active incidents.</p> : null}</div></section></div>
    <section className="form-card"><div className="split-header"><div><h3>Hot route performance</h3><p className="legend">Bounded in-process samples. Paths are normalized; identifiers, query strings, and cookies are not retained.</p></div><div className="legend">Game data gate {formatNumber(routePerformance.gates?.gameData?.active)}/{formatNumber(routePerformance.gates?.gameData?.maxConcurrent)} active · {formatNumber(routePerformance.gates?.gameData?.queued)} queued · {formatNumber(routePerformance.gates?.gameData?.rejected)} rejected<br />Market gate {formatNumber(routePerformance.gates?.market?.active)}/{formatNumber(routePerformance.gates?.market?.maxConcurrent)} active · {formatNumber(routePerformance.gates?.market?.queued)} queued · {formatNumber(routePerformance.gates?.market?.rejected)} rejected</div></div><div className="table-scroll"><table><thead><tr><th>Route</th><th>Samples</th><th>p50</th><th>p95</th><th>p99</th><th>p99 payload</th><th>p99 projection</th><th>429</th></tr></thead><tbody>{(routePerformance.routes ?? []).map((item: AnyRecord) => <tr key={item.path}><td><code>{item.path}</code></td><td>{formatNumber(item.sampleCount)}</td><td>{formatNumber(item.durationMs?.p50)} ms</td><td>{formatNumber(item.durationMs?.p95)} ms</td><td>{formatNumber(item.durationMs?.p99)} ms</td><td>{bytes(item.responseBytes?.p99)}</td><td>{formatNumber(item.projectionMs?.p99)} ms</td><td>{formatNumber(item.status429)}</td></tr>)}</tbody></table></div><p className="legend">{Object.entries(routePerformance.rateLimits ?? {}).map(([name, profile]) => `${name}: ${(profile as AnyRecord).reportOnly ? "report-only" : "enforced"}, ${formatNumber((profile as AnyRecord).wouldLimit)} would-limit`).join(" · ") || "Report-only client profiles have not recorded any would-limit decisions."}</p></section>
    <section className="form-card"><div className="split-header"><div><h3>Operational history retention</h3><p className="legend">Deletion remains disabled with an empty approved-table allowlist.</p></div><span className="server-health-freshness">{formatNumber(operationalHistory.configuredDays ?? 365)} days · {operationalHistory.enabled ? "enabled" : "disabled"}</span></div><div className="table-scroll"><table><thead><tr><th>Table</th><th>Rows</th><th>Oldest</th><th>Eligible</th></tr></thead><tbody>{(operationalHistory.tables ?? []).map((item: AnyRecord) => <tr key={item.table}><td><code>{item.table}</code></td><td>{formatNumber(item.rowCount)}</td><td>{item.oldestOccurredAt ? new Date(item.oldestOccurredAt).toLocaleString() : "-"}</td><td>{formatNumber(item.eligibleRows)}</td></tr>)}</tbody></table></div><p className="legend">Database {bytes(operationalHistory.databaseBytes)} · WAL {bytes(operationalHistory.walBytes)} · Last dry-run {operationalHistory.lastDryRun?.completed_at ? new Date(operationalHistory.lastDryRun.completed_at).toLocaleString() : "not recorded"} · Last prune {operationalHistory.lastPrune?.completed_at ? new Date(operationalHistory.lastPrune.completed_at).toLocaleString() : "never"} · Backup verification {operationalHistory.backupVerification?.integrityCheck === "ok" ? "passed" : "not recorded"}</p></section>
