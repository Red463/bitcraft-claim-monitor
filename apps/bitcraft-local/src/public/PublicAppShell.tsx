...
export function PublicAppShell({ route }: { route: Route }) { const settlement = ["settlement", "members", "inventory", "crafts"].includes(route.id); const identity = route.id === "account" || route.id === "settings"; const legal = route.id === "terms" || route.id === "privacy"; const planAccess = route.id === "shared-plan" || route.id === "invite"; const planWorkspace = ["plans", "plan-new", "plan"].includes(route.id); return <div className="public-app-shell"><Navigation route={route} /><main className="public-main">{!identity && !legal && !planWorkspace && !planAccess ? <SearchPanel /> : null}{settlement ? <SnapshotPage route={route} /> : route.id === "calculator" ? <Calculator /> : identity ? <PublicAccountSettings page={route.id as "account" | "settings"} /> : legal ? <PublicLegalPage type={route.id as "terms" | "privacy"} /> : planAccess ? <PublicPlanAccessPage route={route as { id: "shared-plan" | "invite"; params: Record<string, string> }} /> : planWorkspace ? <PublicPlansPage route={route as { id: "plans" | "plan-new" | "plan"; params: Record<string, string> }} /> : <Placeholder route={route} />}</main></div>; }
