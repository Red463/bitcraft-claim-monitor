...
function Placeholder({ route }: { route: Route }) { return <section className="public-panel public-placeholder"><h1>{title(route)}</h1><p>{route.id === "help" ? "Use settlement search to open current public state. Data refreshes while this page is visible." : "Search for a settlement to view its current public state. Claim Monitor does not continuously monitor public settlements."}</p></section>; }
...
export function PublicAppShell({ route, features = defaultFeatures }: { route: Route; features?: PublicFeatures }) {
  ...
  return <div className="public-app-shell"><Navigation route={route} collaborationEnabled={features.publicCollaborationEnabled} /><main className="public-main">{!identity && !legal && !planWorkspace && !planAccess ? <SearchPanel /> : null}{collaborationRoute && !features.publicCollaborationEnabled ? <section className="public-panel public-placeholder"><h1>Collaboration is not enabled yet</h1><p>Public settlement search and current-state pages remain available.</p></section> : settlement ? <SnapshotPage route={route} /> : route.id === "calculator" ? <Calculator /> : identity ? <PublicAccountSettings page={route.id as "account" | "settings"} /> : legal ? <PublicLegalPage type={route.id as "terms" | "privacy"} /> : planAccess ? <PublicPlanAccessPage route={route as { id: "shared-plan" | "invite"; params: Record<string, string> }} /> : planWorkspace ? <PublicPlansPage route={route as { id: "plans" | "plan-new" | "plan"; params: Record<string, string> }} /> : <Placeholder route={route} />}</main></div>;
}
