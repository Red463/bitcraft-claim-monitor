...
| Current Empire Hexite reserve projection | no dedicated table | Immediate projection from committed regional Empire and global Foundry generations | The Empire treasury and completed `empire_foundry_state` Capsules are aggregated by exact Empire ID and published immediately as a known minimum whenever either source changes. Player/claim inventory amounts remain explicitly unavailable until bounded typed joins are proven. The UI never waits for the retired six-hour sweep and never treats missing sources as zero. |
