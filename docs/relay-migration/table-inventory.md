...
| `public_user_accounts`, `public_user_sessions`, `public_user_legal_acceptances` | keep-user | Public Claim Monitor OAuth, authenticated requests, and privacy workflows | Public-profile identity, session, and legal-acceptance data; independent of game-data provider and deliberately isolated from the Timbersteel identity tables. |
| `public_craft_plans`, `public_craft_plan_members`, `public_craft_plan_invites`, `public_craft_plan_share_links`, `public_craft_plan_events` | keep-user | Authenticated public-plan mutations and claim-scoped public computation reads | User-owned collaborative plan documents, ACLs, HMAC-only invitation/share capabilities, and redacted lifecycle events. Computation reads current Relay settlement inventory and current/completed crafts on demand; these tables never cache game data or alter Timbersteel `craft_plan_settings`. |
