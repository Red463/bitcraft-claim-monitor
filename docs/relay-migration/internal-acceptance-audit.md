...
| Private checkout home isolation | Pass internally; Linux bootstrap pending | The updater defaults its runtime-user Git home to `/opt/bitcraft-claim-monitor-relay` and applies it to fetch, reachability validation, worktree creation/removal, and pruning. A fake-account integration test proves those operations ignore both the Unix account home and maintained deployment paths. The runbook stores the read-only identity, pinned host keys, and SSH config only under the Relay application root. |
