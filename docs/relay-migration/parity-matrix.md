...
| Global market tools | Market Browse catalog search and complete buy/sell order books, Overview current liquidity/hubs/open-order activity, Deals arbitrage, and Barter Stalls now read the live normalized catalog plus bounded typed regional sessions through provider-neutral local routes. Stall trade/building/nickname/location/claim/owner joins publish in the existing `regional-market` generation and add no SQL cache or scheduled job. Exact decimal values, configured-region scope, per-region freshness, generation invalidation, and last-good behavior are preserved. The legacy global-insight job/table and stalls diagnostic path are retired. Completed trades, movers, volume, and cross-market distance remain explicitly unavailable until their authoritative signals are proven. Price Finder, player-trade, and Deal Watch worker paths still require migration | Local catalog/order aggregation, authoritative observed history, bounded location joins, and adaptive region pool | in progress |
