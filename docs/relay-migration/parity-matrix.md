...
| Global market tools | Market Browse catalog search and complete buy/sell order books, Overview current liquidity/hubs/open-order activity, Deals arbitrage, Barter Stalls, and Deal Watch now read the live normalized catalog plus bounded typed regional sessions through provider-neutral local routes. Deal Watch evaluates every committed `regional-market` generation immediately against a clearly labelled current sell-order median; its 30-minute job is reconciliation only, and its legacy price-history/claim-list crawl is removed. User watch configuration and alert/delivery history remain durable, but no Deal Watch current-state table exists. Exact decimal values, item/cargo identity, configured-region scope, generation invalidation, and last-good behavior are preserved. Completed trades, movers, volume, and cross-market distance remain explicitly unavailable until their authoritative signals are proven. Remaining Price Finder and player-trade paths still require migration | Local catalog/order aggregation, authoritative observed history, bounded location joins, and adaptive region pool | in progress |
