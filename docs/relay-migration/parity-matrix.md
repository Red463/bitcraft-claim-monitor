...
| Map/layout | Resource Finder now reads continuously subscribed Relay `resource_desc` and `enemy_desc` rows from the normalized global catalog, refreshes on `catalogs` generation events, and retains durable last-good data without a map cache table. Claim layout, live player locations, and active-region discovery still use legacy paths | Global resource/enemy descriptions plus claim parents and bounded location rows | in progress |
