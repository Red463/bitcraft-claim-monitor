...
| Map/layout | Resource Finder reads continuously subscribed Relay `resource_desc` and `enemy_desc` rows from the normalized global catalog and active-region controls read the live global `region` generation. Both refresh on provider generation events and retain durable last-good data without feature cache tables. Claim layout and live player locations still use legacy paths | Global resource/enemy/region rows plus claim parents and bounded location rows | in progress |
