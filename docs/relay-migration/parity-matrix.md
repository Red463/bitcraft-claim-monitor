...
| Catalogs/planner inputs | Typed global item/cargo repository live. Craft Planner settlement members/inventories and active/passive crafts read committed Relay generations, selected-player inventories use the bounded 15-second Relay service with no dedicated SQL cache, workstation presets/requirements join live building and construction-recipe descriptions locally, and workstation progress uses claim-filtered live `building_state`; probability refresh ownership and remaining description reads still require migration | Global and regional typed subscriptions plus normalized current domains | in progress |
