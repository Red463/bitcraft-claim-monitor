...
| Global market tools | Market Browse catalog search and complete buy/sell order books now read the live normalized catalog plus bounded typed regional sessions through provider-neutral local routes. Exact decimal values, configured-region scope, per-region freshness, and last-good behavior are preserved without a scheduled cache table. Completed-trade history is explicitly unavailable until an authoritative signal is proven. Price Finder, Deals, Overview favorites/aggregates, hubs, trade volume, stalls, player-trade, and Deal Watch worker paths still use BitJita and must migrate | Local catalog/order aggregation, authoritative observed history, and adaptive region pool | in progress |
