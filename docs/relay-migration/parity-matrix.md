...
| Global market tools | Market Browse catalog search and complete buy/sell order books, Overview current liquidity/hubs/open-order activity, and Deals arbitrage now read the live normalized catalog plus bounded typed regional sessions through provider-neutral local routes. Exact decimal values, configured-region scope, per-region freshness, generation invalidation, and last-good behavior are preserved without a scheduled cache table. The legacy global-insight job/table are retired. Completed trades, movers, volume, distance, and map actions remain explicitly unavailable until their authoritative trade/location signals are proven. Price Finder, stalls, player-trade, and Deal Watch worker paths still use BitJita and must migrate | Local catalog/order aggregation, authoritative observed history, bounded location joins, and adaptive region pool | in progress |
