...
| Region | Active-region controls now use continuous typed global population/control/name rows, exact configured scope, and persisted Relay topology/subscription health with no BitJita request or five-minute cache. The worker supervises the subscription independently of HTTP readiness, grants a bounded initial-apply grace, performs jittered reconnect/topology rediscovery, fences every old claim-owned runtime before a claim switch, and publishes scope-keyed browser updates through a single-flight local generation check. Region claim lists, detailed regional aggregates, and trade volume remain legacy/unavailable pending their bounded regional mappings | Global region metadata plus configured regional sessions | in progress |
