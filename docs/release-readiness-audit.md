...
- Full notification matrix verification is still required for production started/completed notification generation and market deal alerts, including sound behavior, visible toast dismissal, dedupe, and refresh behavior. Drawer open/closed, read-state, drawer navigation, and user-level listing settings now have representative browser-smoke evidence, but Production-page toast visibility needs a repeat check.
