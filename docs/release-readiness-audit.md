...
| `server.mjs` | Single large Node service, about 9,711 lines after extracting route classification, dependency-free HTTP policy helpers, response writers, cookie helpers, request-origin helpers, CSRF helpers, body parsing helpers, rate-limit helpers, and visitor IP privacy helpers. Owns static serving, BitJita proxy/cache, SQLite schema/migrations, prepared statements, local/admin APIs, app/admin auth, Discord, analytics, collectors, backups, and scheduled jobs. | Needs further deliberate server-module splits. The first safe extractions are `src/server/httpRoutes.mjs`, `src/server/httpResponses.mjs`, `src/server/httpCookies.mjs`, `src/server/httpRequests.mjs`, `src/server/httpCsrf.mjs`, `src/server/httpBodies.mjs`, `src/server/httpRateLimit.mjs`, and `src/server/visitorIp.mjs`; route/auth/persistence boundaries must remain under test before moving larger code. |
...
- Added `src/notifications/verificationMatrix.ts` as the tested release notification matrix for all routed main-app pages, all supported browser notification types, and the dedicated `/bot` exception.
...
- Re-check deployment docs before final release.
...
- Focused notification matrix test: `node --experimental-strip-types --test test/notification-matrix.test.mjs` passed after adding a tested release checklist for all routed pages, supported browser notification types, and the `/bot` exception.
...
- Focused server integration test: `node --experimental-strip-types --test test/server.test.mjs` passed after the admin-role permission fix, route-helper import wiring, GeoIP license-key redaction check, and Discord bot-token redaction check.
- Full production build: `corepack pnpm --filter @workspace/bitcraft-local run build` passed.
- Full app test suite: `corepack pnpm --filter @workspace/bitcraft-local test` passed with 89 tests.
...
- Public-release security/config scan has fresh 2026-06-28 evidence; repeat it after any further deployment, Discord, auth, settings, or environment-file changes before the final release claim.
