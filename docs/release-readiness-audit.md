...
- `git check-ignore` covers local SQLite/database files, root/app `.env` files, local env variants, local backups/uploads/branding, `.codex-dev/`, and `outputs/`.
- Tracked-file scan found no SQLite databases, log files, private keys, or certificate bundles committed; only the root `.env.example` matched the environment/data filename pattern.
- Token/private-key pattern scan found no committed Discord-token, GitHub-token, OpenAI-key, or private-key shaped literals in tracked source/docs outside documented placeholders, a test-only fake OAuth secret, and server secret plumbing.
...
- Targeted secret/config checks covered tracked runtime data, ignored local data/root env/app env/database/backup/upload files, documented secret placeholders, fake test secrets, server secret plumbing, and private-key/token-shaped literals.
