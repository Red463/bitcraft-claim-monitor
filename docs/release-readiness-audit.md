...
| `server.mjs` | Single large Node service, about 9,787 lines after extracting route classification, dependency-free HTTP policy helpers, response writers, cookie helpers, request-origin helpers, and CSRF helpers. Owns static serving, BitJita proxy/cache, SQLite schema/migrations, prepared statements, local/admin APIs, app/admin auth, Discord, analytics, collectors, backups, and scheduled jobs. | Needs further deliberate server-module splits. The first safe extractions are `src/server/httpRoutes.mjs`, `src/server/httpResponses.mjs`, `src/server/httpCookies.mjs`, `src/server/httpRequests.mjs`, and `src/server/httpCsrf.mjs`; route/auth/persistence boundaries must remain under test before moving larger code. |
...
- Extracted server route classification, visitor logging classification, security headers, frontend MIME type mapping, static cache policy, JSON/text/binary response writers, cookie parsing, HttpOnly session-cookie serialization, request-origin derivation, same-origin checks, safe return-path handling, and admin CSRF token matching into focused `src/server/` helper modules with focused tests, giving the first small dependency-free server modules before larger route/auth/database splits.
...
1. Continue extracting dependency-free HTTP helpers after `httpRoutes.mjs`, `httpResponses.mjs`, `httpCookies.mjs`, `httpRequests.mjs`, and `httpCsrf.mjs`: request parsing and rate-limit helpers without changing behavior.
...
- Full app test suite: `corepack pnpm --filter @workspace/bitcraft-local test` passed with 75 tests.
